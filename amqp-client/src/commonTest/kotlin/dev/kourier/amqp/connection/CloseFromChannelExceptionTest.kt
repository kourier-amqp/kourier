...
class CloseFromChannelExceptionTest {
    ...
    @Test
    fun concurrentCloseFromChannelExceptionDoesNotDoubleFire(): Unit = runTest {
    ...
    @Test
    fun decoderExceptionMidCloseDoesNotPreventTerminalState(): Unit = runTest {
        withContext(Dispatchers.Default) {
            val scope = CoroutineScope(coroutineContext + SupervisorJob() + Dispatchers.Default)
            val conn = TestableConnection(amqpConfig {}, scope)
            try {
     ...
     */
    @Test
    fun stressManyConcurrentDecoderExceptionsCoalesce(): Unit = runTest {
        withContext(Dispatchers.Default) {
            repeat(20) { iter ->
                val scope = CoroutineScope(coroutineContext + SupervisorJob() + Dispatchers.Default)
                val conn = TestableConnection(amqpConfig {}, scope)
